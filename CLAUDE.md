...
> En cas de modification, régénérer avec: `forge regenerate-claude bone-annotator`
> Ou via API: POST /api/skills/bone-annotator/regenerate-claude
...
**IMPORTANT**: Votre code sera automatiquement validé par `curl -X POST http://10.0.0.13:4080/api/validate/bone-annotator`
  ...
  "entries": [
    {
      ...
      "paths": ["/opt/onyx/skills/bone-annotator/config/"],
      "criticality": "medium",
      "description": "Configuration du skill (connexions, paramètres)"
    },
    {
      "data_type": "sqlite",
      "paths": ["/opt/onyx/skills/bone-annotator/data/app.db"],
      "criticality": "medium",
...
☐ curl -X POST http://10.0.0.13:4080/api/validate/bone-annotator = valid: true
...
Chaque skill dispose d'une bibliographie alimentée depuis Zotero (collection `forge/bone-annotator`)
...
- **Qdrant collection `forge_biblio_bone-annotator`** — recherche sémantique vectorielle (OnyxSoma)
...
Chaque skill a sa propre collection Qdrant : `forge_biblio_bone-annotator` (ex: `forge_biblio_cr_engine`).
