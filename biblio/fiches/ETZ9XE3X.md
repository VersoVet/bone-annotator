...
{'study_design': 'Revue narrative/systématique de la littérature avec recherche par mots-clés combinant termes de segmentation ("segmentation", "Segment Anything", "SAM") et termes médicaux ("medical", "clinical", "radiology", "CT", "MRI", "pathology", "chest X-ray", "ultrasound"), ainsi que des noms de méthodes ("text-prompted", "referring segmentation", "reasoning segmentation"); screening par titre/résumé puis texte complet', 'intervention': None, 'control': None, 'primary_outcomes': ["Taxonomie des modèles vision-langage pour la segmentation d'images médicales", 'Comparaison des performances (Dice) et caractéristiques des méthodes'], 'secondary_outcomes': ['Statut de peer-review et disponibilité du code', 'Qualité des preuves (validation externe/multi-site, risque de fuite de données)'], 'statistical_methods': [], 'duration': 'Littérature couvrant la période 2021 (sortie de CLIP) à janvier 2026, avec emphase sur 2024-2026', 'setting': "Revue de littérature scientifique (pas d'étude clinique primaire)"}

## Résultats

{'quantitative': [{'outcome': 'Taille du jeu de données MIMIC-CXR', 'value': '370,000', 'unit': 'images avec rapports en texte libre', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Chest Radiography', 'source_quote': 'MIMIC-CXR alone contains more than 370,000 images with free-text reports'}, {'outcome': 'Score Dice médian de BiomedParse', 'value': '>90%', 'unit': 'Dice score', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Text-Prompt-Guided Segmentation', 'source_quote': 'reaching a median Dice score above 90% and outperforming bounding-box methods on irregular shapes'}, {'outcome': 'Paires image-masque MedSAM', 'value': '1.5 million', 'unit': 'paires image-masque, 10 modalités', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Segment Anything and Medical Variants', 'source_quote': 'It collected more than 1.5 million image-mask pairs across ten modalities and fine-tuned the SAM mask decoder while keeping the image encoder mostly frozen, using bounding-box prompts.'}, {'outcome': 'Échelle SAM-Med2D', 'value': '4.6 million images, 19.7 million masks', 'unit': 'images/masques', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Segment Anything and Medical Variants', 'source_quote': 'SAM-Med2D scaled the dataset to roughly 4.6 million images and 19.7 million masks and added encoder adapter layers to bridge the natural-medical gap'}, {'outcome': 'Paramètres mis à jour par SAM2LoRA', 'value': '<5%', 'unit': 'pourcentage de paramètres', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Segment Anything and Medical Variants', 'source_quote': 'SAM2LoRA reaches state-of-the-art retinal fundus segmentation while updating fewer than five percent of the parameters'}, {'outcome': "Triplets d'entraînement BiomedParseData", 'value': '6 millions', 'unit': 'triplets image-masque-description', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Benchmark Datasets', 'source_quote': 'BiomedParseData provides six million image-mask-description triples harmonized from forty-five datasets with GPT-4'}, {'outcome': "Paramètres de l'encodeur SAM", 'value': '>600 million', 'unit': 'paramètres', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Efficiency and 3D Scaling', 'source_quote': "SAM's largest encoder alone exceeds six hundred million parameters"}], 'qualitative_findings': ["Les modèles vision-langage transforment la segmentation d'images médicales en remplaçant les prompts visuels (points, boîtes) par des descriptions en langage naturel", "Les modèles universels guidés par texte permettent de segmenter des dizaines à des centaines d'organes/lésions avec un seul réseau", 'La grande majorité des modèles restent des prototypes de recherche évalués rétrospectivement, non validés cliniquement de façon prospective', 'Le peer-review est mixte parmi les méthodes largement citées, plusieurs restant des preprints'], 'main_findings': ['Une taxonomie en trois catégories est proposée : modèles guidés par prompt textuel, architectures intégrant des LLM, et cadres hybrides', 'SAM et ses variantes médicales (MedSAM, SAM-Med2D, SAM-Med3D, SAM 2/3, MedSAM2/3) constituent une lignée majeure mais restent limitées par leur dépendance aux prompts visuels', 'Des modèles comme BiomedParse, BiomedParse-V, SAT et CLIP-Driven Universal Model permettent une segmentation multi-modalités et multi-structures via texte', 'Quatre défis ouverts sont identifiés : dépendance au prompt, hallucination de masques, inférence volumétrique lente, et données annotées limitées', "La radiographie thoracique dispose de l'écosystème vision-langage le plus mature grâce à de grands jeux de données appariés image-rapport", "L'échographie reste difficile en raison de la rareté des grands jeux de données image-texte et de la dépendance à l'opérateur"]}

## Conclusions

La période 2024-2026 a produit plus d'avancées que la décennie précédente en segmentation d'images médicales guidée par le langage Des modèles comme BiomedParse, MedSAM2 et SAT segmentent désormais de nombreuses modalités et structures à partir de prompts textuels, tandis que LoRA et les adaptateurs rendent le déploiement pratique Quatre défis ouverts (dépendance au prompt, hallucination, vitesse d'inférence, rareté des données) définissent une feuille de route de recherche vers un déploiement fiable et bien intégré Le potentiel gain clinique est substantiel (réduction du temps et coût de segmentation, cohérence accrue entre institutions, nouvelles applications comme le rapport radiologique ancré et l'aide à la décision conversationnelle) mais reste largement à démontrer par une validation clinique prospective plutôt que par des benchmarks rétrospectifs
