...
{'study_design': "Cadre d'entraînement noise-robust combinant un module d'adaptation au bruit (noise adaptor) qui infère les probabilités de corruption pixel par pixel et repondère la supervision non fiable, avec une stratégie de cohérence MPVT (multi-stage perturbation and variable-teacher) assemblant un ensemble de modèles enseignants stochastiques appliquant des perturbations progressivement plus strictes", 'intervention': "Application du framework MPVT+ (noise adaptor + consistance MPVT) à l'entraînement d'un réseau de segmentation sur des données CT hépatiques avec labels bruités", 'control': "U-Net entraîné sans gestion du bruit (baseline 'noise-free U-Net')", 'primary_outcomes': ['Précision de la segmentation des tumeurs hépatiques mesurée par DSC (Dice Similarity Coefficient)'], 'secondary_outcomes': ['JSC (Jaccard Similarity Coefficient)', 'SVD', 'VOE (Volumetric Overlap Error)'], 'statistical_methods': [], 'duration': None, 'setting': None}

## Résultats

{'quantitative': [{'outcome': 'Dice Similarity Coefficient (DSC) - baseline U-Net sans gestion du bruit', 'value': '75.1', 'unit': '%', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Abstract / Discussion', 'source_quote': 'Compared to a noise-free U-Net baseline (Dice Similarity Coefficient [DSC] 75.1%), MPVT+ improved segmentation accuracy to 80.3%.'}, {'outcome': 'Dice Similarity Coefficient (DSC) - MPVT+', 'value': '80.3', 'unit': '%', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Abstract / Discussion', 'source_quote': 'MPVT+ achieved the Dice similarity coefficient (DSC) from 75.1 % for a noise-free U-Net baseline to 80.3 %.'}], 'qualitative_findings': [], 'main_findings': ['MPVT+ surpasse significativement les approches de référence et les méthodes traditionnelles de gestion du bruit', "Le framework obtient des résultats supérieurs de manière cohérente sur plusieurs métriques d'évaluation (DSC, JSC, SVD, VOE)"]}

## Conclusions

Le framework proposé intègre une architecture robuste avec des techniques de régularisation par cohérence pour atténuer l'impact négatif des labels bruités sur l'entraînement des DNNs En incorporant des données étiquetées propres et bruitées, l'approche améliore la robustesse au bruit et la capacité de généralisation du modèle, surpassant les méthodes traditionnelles La méthode réduit le besoin de construire un jeu de données parfaitement étiqueté pour l'entraînement des DNNs, servant d'outil précieux pour aider à la prise de décision clinique
