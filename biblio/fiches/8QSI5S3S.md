...
{'study_design': 'Entraînement, validation et test de modèles YOLOv8 (différentes tailles) sur le dataset GRAZPEDWRI-DX, avec et sans augmentation de données, et comparaison des performances avec YOLOv7 et ses variantes améliorées (CBAM, GAM).', 'intervention': "Augmentation de données par ajustement du contraste et de la luminosité des images X-ray via la fonction addWeighted d'OpenCV (α=1.2, γ=30), étendant le training set de 14 204 à 28 408 images ; architecture YOLOv8 modifiée (backbone CSP avec module C2f, SPPF, neck FP-PAN, tête découplée Anchor-Free, Task Aligned Assigner, pertes BCE/DFL/CIoU).", 'control': 'Modèle YOLOv8 original (sans augmentation de données) et modèles YOLOv7 / YOLOv7-CBAM / YOLOv7-GAM', 'primary_outcomes': ['mean Average Precision (mAP 50)'], 'secondary_outcomes': ["Temps d'inférence (ms)", 'Nombre de paramètres', 'FLOPs', 'Précision', 'Rappel', 'F1-score'], 'statistical_methods': ['Intersection over Union (IoU)', 'Precision-Recall Curve', 'F1-score', 'mAP 50'], 'duration': "100 époques d'entraînement (ajusté depuis 200 époques initiales avec patience de 50)", 'setting': 'Entraînement sur GPU unique GeForce RTX 3080Ti 12GB, Python 3.8, PyTorch 1.8.2, dataset public GRAZPEDWRI-DX'}

## Résultats

{'quantitative': [{'outcome': 'mAP 50 du modèle proposé vs YOLOv7 amélioré et YOLOv8 original', 'value': '0.638 (modèle proposé) vs 0.634 (YOLOv7 amélioré) vs 0.636 (YOLOv8 original)', 'unit': 'mAP 50', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Abstract', 'source_quote': 'Specifically, mAP 50 of our model is 0.638, which is significantly higher than the 0.634 and 0.636 of the improved YOLOv7 and original YOLOv8 models.'}, {'outcome': 'mAP 50 de la classe bone-anomaly avant/après augmentation de données', 'value': '0.11 → 0.169', 'unit': 'mAP 50', 'confidence_interval': None, 'p_value': None, 'effect_size': 'augmentation de 53.6%', 'source_section': 'Ablation study', 'source_quote': 'Compared with YOLOv8s model, the mAP value predicted by the model using our training method for bone-anomaly increased from 0.11 to 0.169, an increase of 53.6%.'}, {'outcome': "mAP 50 YOLOv8m et YOLOv8l avant/après augmentation de données (taille d'image 640)", 'value': 'YOLOv8m: 0.621 → 0.629 ; YOLOv8l: 0.623 → 0.637', 'unit': 'mAP 50', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Experimental results', 'source_quote': 'Specifically, when the input image size is 640, compared with YOLOv8m model and YOLOv8l model, the mAP 50 of our model improves from 0.621 to 0.629, and from 0.623 to 0.637, respectively.'}, {'outcome': "Temps d'inférence CPU YOLOv8m et YOLOv8l avant/après augmentation de données", 'value': 'YOLOv8m: 536.4 → 685.9 ms ; YOLOv8l: 1006.3 → 1370.8 ms', 'unit': 'ms', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Experimental results', 'source_quote': 'Although the inference time on the CPU is increased from 536.4 ms and 1006.3 ms to 685.9 ms and 1370.8 ms, respectively, the number of parameters and FLOPs are the same'}, {'outcome': 'Comparaison SGD vs Adam pour YOLOv8m (input 1024) - meilleure époque', 'value': 'SGD: époque 35 ; Adam: époque 70', 'unit': 'époque', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Experimental results', 'source_quote': 'Specifically, for YOLOv8m model with an input image size of 1024, the model trained with the SGD optimizer achieves the best performance at the 35th epoch, while the best performance of the model trained with the Adam optimizer is at the 70th epoch.'}, {'outcome': 'Comparaison SGD vs Adam pour YOLOv8s (input 640)', 'value': "mAP SGD supérieur de 0.007 à Adam ; temps d'inférence SGD plus lent de 0.1ms", 'unit': 'mAP / ms', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Experimental results', 'source_quote': 'when the input image size is 640, the mAP value of YOLOv8s model trained with the SGD optimizer is 0.007 higher than that of the model trained with the Adam optimizer, while the inference time is 0.1ms slower.'}, {'outcome': 'Répartition du dataset en training/validation/test', 'value': '14204 (69.88%) / 4094 (20.14%) / 2029 (9.98%)', 'unit': 'images', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Ethics approval / Methods', 'source_quote': 'our training set consists of 14,204 images (69.88%), our validation set consists of 4,094 images (20.14%), and our test set consists of 2029 images (9.98%)'}], 'qualitative_findings': ['Le modèle a une bonne capacité à détecter les fractures dans les cas de fracture unique, mais la ponction métallique et les situations de fractures multiples denses affectent négativement la précision de la prédiction.'], 'main_findings': ['Le modèle proposé (YOLOv8 avec augmentation de données) atteint un mAP 50 de 0.638, supérieur à YOLOv7 amélioré (0.634) et YOLOv8 original (0.636), constituant une performance state-of-the-art (SOTA)', "L'augmentation de données (ajustement du contraste/luminosité) améliore particulièrement la détection de la classe bone-anomaly, la plus difficile à détecter", "L'optimiseur Adam est plus adapté pour l'entraînement sur le petit dataset original (14204 images), tandis que SGD est préféré après augmentation de données (28408 images)", "Une application GUI 'Fracture Detection Using YOLOv8 App' a été développée pour assister les chirurgiens pédiatriques"]}

## Conclusions

L'application de YOLOv8 combiné à l'augmentation de données à la détection de fractures du poignet pédiatrique permet d'atteindre une performance state-of-the-art (mAP 50 = 0.638) L'application développée peut aider les chirurgiens pédiatriques, notamment dans les régions sous-équipées, à interpréter les images radiographiques et réduire les erreurs de diagnostic Le modèle entraîné peut servir de modèle pré-entraîné pour la détection de fractures dans d'autres parties du corps
