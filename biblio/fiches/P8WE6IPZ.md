...
{'study_design': "Entraînement et évaluation d'un modèle hybride de segmentation (CervSpineNet) sur trois variantes de dataset (Original, CLAHE, Augmenté), avec comparaison à quatre modèles de référence", 'intervention': 'Application du modèle CervSpineNet, intégrant un encodeur basé sur transformer pour capturer le contexte anatomique global et un décodeur convolutif léger pour affiner les frontières locales, entraîné avec une fonction de perte composite (Dice, Focal Tversky, Hausdorff distance transform, SSIM)', 'control': 'Quatre modèles de référence (baselines): U-Net, DeepLabV3+, Segment Anything Model (SAM), et SegFormer guidé par texte', 'primary_outcomes': [], 'secondary_outcomes': [], 'statistical_methods': [], 'duration': None, 'setting': 'Système unique avec PyTorch et un GPU NVIDIA H100 Tensor Core; comparaison également avec CPU Intel® Xeon® Gold 6136 (2 × 12 cœurs)'}
...
CervSpineNet, un framework hybride transformer-CNN, réalise une segmentation automatisée des apophyses épineuses cervicales sur des radiographies latérales, accompagné du premier jeu de données organisé spécifiquement pour cette tâche Le couplage d'un encodeur ViT-B capturant le contexte anatomique global et d'un décodeur convolutif léger optimisé pour la reconstruction fine des structures permet à CervSpineNet d'atteindre de fortes performances, avec des scores de Dice moyens dépassant 0,93 et des valeurs SSIM supérieures à 0,98 sur plusieurs variantes du jeu de données Les comparaisons statistiques démontrent des améliorations significatives par rapport aux références établies Le modèle réduit le temps d'annotation manuelle d'environ 96%, produisant des masques binaires précis en 5 à 10 secondes sur du matériel clinique standard Avec sa haute précision, son empreinte computationnelle compacte et son jeu de données librement accessible, CervSpineNet offre une base pratique et reproductible pour l'intégration clinique future et la recherche méthodologique Le framework a des applications potentielles en imagerie de la colonne vertébrale, planification chirurgicale, surveillance postopératoire et radiologie quantitative, et fournit une plateforme évolutive pour l'analyse automatisée de l'anatomie cervicale
