...
{'study_design': 'Development and benchmarking of a SAM-based segmentation framework (LDFSAM) using a lightweight prompt generator refined via Localization Distillation (LD) to produce Dense Feature Prompts (DFP) and Sparse Feature Prompts (SFP) injected into the SAM decoder; compared against SAM, MedSAM, SAMed, and SAM-Med2D across varying annotation budgets (10%, 20%, 50%, 100%)', 'intervention': 'LDFSAM: latent feature prompting using DFP and SFP generated by a lightweight prompt generator (YOLOv8n student distilled from a YOLOv8x teacher via Localization Distillation) instead of explicit bounding box/point prompts', 'control': 'SAM, MedSAM, SAMed, and SAM-Med2D as comparison SAM-based baselines, plus conventional (task-specific) segmentation networks', 'primary_outcomes': ['Dice score for segmentation accuracy across datasets and annotation budgets'], 'secondary_outcomes': ['Parameter counts and computational cost (GPU hours, GFLOPs, FPS)', 'Qualitative segmentation quality (boundary adherence, lesion shape capture, suppression of spurious foreground)'], 'statistical_methods': ['Five independent runs using fixed random seeds ({10, 42, 123, 2025, 3407})', '95% confidence intervals estimated via non-parametric bootstrap resampling (B = 1000)'], 'duration': None, 'setting': 'Four public benchmark datasets (3D CBCT Tooth, ISIC 2018, MMOTU, Kvasir-SEG) and one private in-house CBCT dataset'}

## Résultats

{'quantitative': [{'outcome': 'Dice score across four public benchmarks', 'value': '> 0.91', 'unit': 'Dice score', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Abstract', 'source_quote': 'LDFSAM outperforms both prior SAM-based baselines and conventional networks, achieving Dice scores exceeding 0.91.'}, {'outcome': 'Automatic prompt generation branch parameter count', 'value': '3.3', 'unit': 'million parameters', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Methods', 'source_quote': 'The automatic prompt generation branch operates at 640 × 640 resolution and contains approximately 3.3 M parameters with about 9.4 GFLOPs per image, running at ~122 FPS under our profiling setup.'}, {'outcome': 'SAM segmentation network parameter count (excluding prompt branch)', 'value': '271.2', 'unit': 'million parameters', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Methods', 'source_quote': 'Excluding this branch, our SAM segmentation network has approximately 271.2 M parameters.'}, {'outcome': 'Additional parameter overhead of prompt branch relative to segmentation model', 'value': '1.2', 'unit': '%', 'confidence_interval': None, 'p_value': None, 'effect_size': None, 'source_section': 'Methods', 'source_quote': 'Therefore, the proposed prompt branch adds only about 1.2% additional parameters relative to the segmentation model, resulting in a modest computational and memory overhead while enabling fully automatic feature-based prompting.'}], 'qualitative_findings': ['LDFSAM yields masks that adhere more closely to fine tooth boundaries in CBCT slices, better capture irregular lesion shapes in dermoscopy images, and suppress spurious foreground blobs in colonoscopy scenes compared with other SAM-based methods'], 'main_findings': ['Latent feature-level prompts (DFP and SFP) substantially outperform geometric box prompts', 'Combining DFPs and SFPs yields consistent gains over using either alone', 'LDFSAM surpasses existing SAM-based baselines across different annotation budgets and achieves Dice scores competitive with or superior to strong task-specific architectures', 'The advantage of LDFSAM is particularly pronounced in low-annotation regimes (10-20%), with minimal overlap in 95% confidence intervals versus competing SAM-based methods', 'As training data proportion increases, the performance gap to other SAM-based methods narrows but remains consistent, with LDFSAM showing narrower shaded areas reflecting superior stability', 'Models with larger parameters (SAM-Med2D and LDFSAM) achieved lower GPU hours across all four datasets compared to lightweight models (SAM, MedSAM, SAMed)', 'LDFSAM maintains training time cost overhead comparable to SAM-Med2D while effectively handling complex segmentation tasks']}

## Conclusions

LDFSAM enhances SAM-based medical segmentation by replacing geometric prompts with localization-distilled feature prompts delivered in the latent space Feature-based latent prompts substantially outperform box prompts, and combining DFP and SFP yields consistent gains over using either alone Refining the prompt generator via localization-aware distillation (LD) is crucial for stable and accurate segmentation Under varying annotation budgets, LDFSAM consistently surpasses existing SAM-based methods, especially in low-label regimes, and achieves Dice scores competitive with or superior to strong task-specific segmentation networks Additional experiments on a private in-house CBCT dataset show similar improvements, indicating good generalization beyond public benchmarks Carefully designed feature-level latent prompts distilled from a localization model can narrow the gap between generic foundation models and specialized medical segmentation architectures while keeping inference lightweight and modular
