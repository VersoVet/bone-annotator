...
{'study_design': "Revue narrative/technique de la littérature sur les solutions de deep learning traitant deux limitations majeures des jeux de données : annotations rares (scarce annotations) et annotations faibles (weak annotations, incluant annotations éparses, bruitées ou au niveau de l'image)", 'intervention': None, 'control': None, 'primary_outcomes': [], 'secondary_outcomes': [], 'statistical_methods': [], 'duration': None, 'setting': 'Littérature de recherche en imagerie médicale et apprentissage profond'}

## Résultats

{'quantitative': [], 'qualitative_findings': ['Les solutions pour les annotations rares sont classées en trois catégories selon les besoins en données : faibles, moyens et élevés', 'Le post-traitement basé sur CRF montre des résultats mitigés pour la segmentation 3D', "Les représentations d'images 3D modifiées apportent des gains modérés au prix de l'entraînement de plusieurs modèles 2D", "L'apprentissage semi-supervisé avec pseudo-annotations montre des résultats mitigés, sauf pour les architectures avancées gérant le bruit d'annotation", "Les techniques d'adaptation de domaine sont efficaces mais difficiles à mettre en œuvre en raison de l'instabilité de l'entraînement adversarial", "L'apprentissage semi-supervisé sans pseudo-annotations nécessite uniquement des données non annotées supplémentaires et est moins exigeant à mettre en œuvre", "L'apprentissage multi-tâches et la fusion de jeux de données sont des solutions simples avec des gains de performance raisonnables", "Le pré-entraînement auto-supervisé est considéré comme l'une des approches les plus prometteuses", "L'apprentissage actif et la segmentation interactive sont recommandés lorsque l'annotation de données supplémentaires par des experts est nécessaire"], 'main_findings': ['Les limitations des jeux de données de segmentation médicale se répartissent en deux catégories : annotations rares et annotations faibles', "Une combinaison de solutions individuelles peut être utilisée pour traiter des jeux de données combinés souffrant à la fois d'annotations rares et faibles", 'Un tableau récapitulatif (Table 7) classe les méthodologies selon leurs exigences en données et peut servir de guide de stratégie pour choisir la méthode appropriée']}

## Conclusions

Pour le problème des annotations rares, un ensemble diversifié de solutions existe, allant des solutions semi-automatisées avec experts humains (apprentissage actif, segmentation interactive) aux solutions entièrement automatisées exploitant des données non labellisées ou synthétiques Pour le problème des annotations faibles, des solutions existent pour gérer les annotations éparses, bruitées ou uniquement au niveau de l'image Les méthodologies ont été comparées en termes de ressources de données requises, de difficulté de mise en œuvre et de gains de performance, en mettant en avant celles offrant le meilleur compromis coût-bénéfice Cette revue vise à accroître la sensibilisation de la communauté aux stratégies de gestion des annotations rares et faibles et à encourager davantage de recherche dans ce domaine
