...
{'study_design': "Développement et évaluation expérimentale d'un algorithme d'annotation semi-automatique en trois parties : (1) un réseau de base EGFPNet (Edge Guidance Feature Pyramid Network) intégrant l'information des contours pour améliorer la qualité de segmentation des bords, (2) un algorithme de segmentation par apprentissage semi-faiblement supervisé entraîné avec quelques labels de segmentation épars et un grand nombre de labels de type bounding box, (3) une stratégie d'annotation itérative basée sur l'apprentissage actif pour la mise à jour itérative du modèle et de l'annotation", 'intervention': "Algorithme semi-weakly supervised learning combinant labels de segmentation épars et labels de type bounding box pour entraîner EGFPNet, avec stratégie d'annotation itérative basée sur l'apprentissage actif", 'control': "Algorithme d'apprentissage entièrement supervisé (fully supervised learning) utilisé comme comparaison", 'primary_outcomes': ['Performance de pré-annotation (pre-annotation performance) du modèle'], 'secondary_outcomes': [], 'statistical_methods': [], 'duration': None, 'setting': None}

## Résultats

{'quantitative': [], 'qualitative_findings': ["L'algorithme proposé atteint une performance de pré-annotation équivalente à celle de l'algorithme d'apprentissage entièrement supervisé, alors que le nombre de labels de segmentation utilisés est bien inférieur"], 'main_findings': ["Les résultats expérimentaux sur des jeux de données publics d'IRM montrent que l'algorithme proposé atteint une performance de pré-annotation équivalente à celle des algorithmes d'apprentissage entièrement supervisé, avec un nombre de labels de segmentation nettement inférieur"]}

## Conclusions

L'algorithme d'annotation semi-automatique d'IRM basé sur l'apprentissage semi-faiblement supervisé résout le problème de faible performance de pré-annotation des algorithmes existants en cas de labels de segmentation insuffisants L'algorithme de segmentation semi-faiblement supervisé basé sur des labels épars, combiné à quelques labels de segmentation épars et un grand nombre de labels bounding box, permet d'entraîner efficacement EGFPNet La stratégie d'annotation itérative basée sur l'apprentissage actif permet une mise à jour itérative du modèle et de l'annotation Les tests sur un jeu de données IRM public prouvent l'efficacité de l'algorithme proposé
